...
class FootballEventSchemaBase(BaseModel):
    ...
    play_direction: Annotated[str, Path(max_length=150)] | None = Field(
        None, examples=["left", "right", "middle"]
    )
    event_strong_side: Annotated[str, Path(max_length=150)] | None = Field(
        None, examples=["left", "right"]
    )
    play_type: Annotated[str, Path(max_length=150)] | None = Field(
        None, examples=["run", "pass", "kick", "punt"]
    )
    play_result: Annotated[str, Path(max_length=150)] | None = Field(
        None, examples=["gain", "loss", "incomplete", "interception"]
    )
    score_result: Annotated[str, Path(max_length=150)] | None = Field(
        None, examples=["touchdown", "field_goal", "none"]
    )
...
FootballEventSchemaUpdate = make_fields_optional(FootballEventSchemaBase)
