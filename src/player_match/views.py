...
from ..player_team_tournament.schemas import PlayerTeamTournamentSchema, PlayerTeamTournamentSchemaCreate
        ...
        async def create_parsed_eesl_match_endpoint(
            ...
            if parsed_match and match:
                ...
                for home_player in parsed_match["roster_a"]:
                    # pprint(home_player)
                    position = await position_service.get_position_by_title(home_player['player_position'])
                    if position is None:
                        position_schema = {'title': home_player['player_position'], 'sport_id': 1}
                        position = await PositionServiceDB(db).create_new_position(
                            PositionSchemaCreate(**position_schema))
                    person = await PersonServiceDB(db).get_person_by_eesl_id(home_player['player_eesl_id'])
                    if person is None:
                        player_in_team = await collect_player_full_data_eesl(home_player["player_eesl_id"])
                        person_schema = PersonSchemaCreate(**player_in_team["person"])
                        person = await PersonServiceDB(db).create_or_update_person(person_schema)

                    player = await PlayerServiceDB(db).get_player_by_eesl_id(home_player['player_eesl_id'])
                    if player is None:
                        ...
                        player = await PlayerServiceDB(db).create_or_update_player(player_schema)
                    ...
                    if player_in_team is None:
                        ...
                        player_in_team = await PlayerTeamTournamentServiceDB(
                            db).create_or_update_player_team_tournament(
                ...
                for away_player in parsed_match["roster_b"]:
                    position = await position_service.get_position_by_title(away_player['player_position'])
                    if position is None:
                        position_schema = {'title': away_player['player_position'], 'sport_id': 1}
                        position = await PositionServiceDB(db).create_new_position(
                            PositionSchemaCreate(**position_schema))
                    person = await PersonServiceDB(db).get_person_by_eesl_id(away_player['player_eesl_id'])
                    if person is None:
                        player_in_team = await collect_player_full_data_eesl(away_player["player_eesl_id"])
                        person_schema = PersonSchemaCreate(**player_in_team["person"])
                        person = await PersonServiceDB(db).create_or_update_person(person_schema)

                    player = await PlayerServiceDB(db).get_player_by_eesl_id(away_player['player_eesl_id'])
                    if player is None:
                        player_schema = PlayerSchemaCreate(**{
                            'sport_id': 1,
                            'person_id': person.id,
