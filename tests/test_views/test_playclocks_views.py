...
class TestPlayClockViews:
    async def test_create_playclock_endpoint(self, client_match, test_db):
        ...
        response = await client_match.post("/api/playclock/", json=playclock_data.model_dump())

        assert response.status_code == 200
        assert response.json()["id"] > 0

    async def test_update_playclock_endpoint(self, client_match, test_db):
        sport_service = SportServiceDB(test_db)
        sport = await sport_service.create(SportFactorySample.build())
        ...
        response = await client_match.put(f"/api/playclock/{created.id}/", json=update_data.model_dump())

        assert response.status_code == 200

    async def test_update_playclock_not_found(self, client_match):
        update_data = PlayClockSchemaUpdate(playclock=120)

        response = await client_match.put("/api/playclock/99999/", json=update_data.model_dump())

        assert response.status_code == 404

    async def test_get_playclock_by_id_endpoint(self, client_match, test_db):
        sport_service = SportServiceDB(test_db)
        sport = await sport_service.create(SportFactorySample.build())
        ...
        response = await client_match.get(f"/api/playclock/id/{created.id}/")

        assert response.status_code == 200
        assert response.json()["content"]["id"] == created.id

    async def test_get_playclock_by_id_not_found(self, client_match):
        response = await client_match.get("/api/playclock/id/99999/")

        assert response.status_code == 404

    async def test_get_all_playclocks_endpoint(self, client_match, test_db):
        sport_service = SportServiceDB(test_db)
        sport = await sport_service.create(SportFactorySample.build())
        ...
        response = await client_match.get("/api/playclock/")

        assert response.status_code == 200
        assert len(response.json()) == 2

    async def test_playclock_update_increments_version(self, client_match, test_db):
    ...
    async def test_playclock_update_websocket_broadcasts_version(self, client_match, test_db):
    ...
    async def test_playclock_stopped_state_reflected_in_ws(self, client_match, test_db):
