...
from tests.factories import TeamFactory, TournamentFactory, SeasonFactorySample, SportFactorySample, PlayerFactory, PersonFactory
...
class TestPlayerTeamTournamentViews:
    async def test_create_player_team_tournament_endpoint(self, client, test_db):
        ...
        ptt_data = PlayerTeamTournamentSchemaCreate(player_id=player.id, position_id=position.id, team_id=team.id, tournament_id=tournament.id, player_team_tournament_eesl_id=100)
    ...
    async def test_get_player_team_tournament_by_eesl_id_endpoint(self, client, test_db):
        ...
        player_service = PlayerServiceDB(test_db)
        player = await player_service.create_or_update_player(PlayerFactory.build(person_id=person.id))

        ptt_service = PlayerTeamTournamentServiceDB(test_db)
        ptt_data = PlayerTeamTournamentSchemaCreate(player_id=player.id, position_id=position.id, team_id=team.id, tournament_id=tournament.id)
    ...
    async def test_get_all_player_team_tournaments_endpoint(self, client, test_db):
        ...
        ptt_service = PlayerTeamTournamentServiceDB(test_db)
        await ptt_service.create(PlayerTeamTournamentSchemaCreate(player_id=player1.id, position_id=position.id, team_id=team.id, tournament_id=tournament.id))
        await ptt_service.create(PlayerTeamTournamentSchemaCreate(player_id=player2.id, position_id=position.id, team_id=team.id, tournament_id=tournament.id))
