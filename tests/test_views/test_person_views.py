...
class TestPersonViews:
    async def test_create_person_endpoint(self, client_player, test_db):
        PersonServiceDB(test_db)
        person_data = PersonFactory.build(person_eesl_id=100)

        response = await client_player.post("/api/persons/", json=person_data.model_dump(mode="json"))

        assert response.status_code == 200
        assert response.json()["id"] > 0

    async def test_get_person_by_eesl_id_endpoint(self, client_player, test_db):
        person_service = PersonServiceDB(test_db)
        person_data = PersonFactory.build(person_eesl_id=100)
        created = await person_service.create_or_update_person(person_data)

        response = await client_player.get("/api/persons/eesl_id/100")

        assert response.status_code == 200
        assert response.json()["id"] == created.id

    async def test_get_person_by_eesl_id_not_found(self, client_player):
        response = await client_player.get("/api/persons/eesl_id/99999")

        assert response.status_code == 404

    async def test_update_person_endpoint(self, client_player, test_db):
        person_service = PersonServiceDB(test_db)
        ...
        response = await client_player.put(f"/api/persons/{created.id}/", json=update_data.model_dump())

        assert response.status_code == 200
        assert response.json()["first_name"] == "Updated Name"

    async def test_get_all_persons_endpoint(self, client_player, test_db):
        person_service = PersonServiceDB(test_db)
        await person_service.create_or_update_person(PersonFactory.build())
        await person_service.create_or_update_person(PersonFactory.build())

        response = await client_player.get("/api/persons/paginated")

        assert response.status_code == 200
        assert len(response.json()["data"]) >= 2

    async def test_get_person_by_id_endpoint(self, client_player, test_db):
        person_service = PersonServiceDB(test_db)
        person_data = PersonFactory.build()
        created = await person_service.create_or_update_person(person_data)

        response = await client_player.get(f"/api/persons/id/{created.id}")

        assert response.status_code == 200
        assert response.json()["id"] == created.id

    async def test_get_person_by_id_not_found(self, client_player):
        response = await client_player.get("/api/persons/id/99999")
        assert response.status_code == 404

    async def test_upload_person_photo_endpoint(self, client_player):
        file_content = create_test_image()
        files = {"file": ("test_photo.png", BytesIO(file_content), "image/png")}
        response = await client_player.post("/api/persons/upload_photo", files=files)

        assert response.status_code == 200
        assert "photoUrl" in response.json()
        assert "persons/photos" in response.json()["photoUrl"]

    async def test_upload_person_photo_with_invalid_file(self, client_player):
    ...
    async def test_upload_and_resize_person_photo_endpoint(self, client_player):
        file_content = create_test_image()
        files = {"file": ("test_photo.png", BytesIO(file_content), "image/png")}
        response = await client_player.post("/api/persons/upload_resize_photo", files=files)
    ...
    async def test_upload_and_resize_person_photo_with_invalid_file(self, client_player):
        file_content = b"not a valid image"
        files = {"file": ("test_invalid.txt", BytesIO(file_content), "text/plain")}
        response = await client_player.post("/api/persons/upload_resize_photo", files=files)
    ...
    async def test_get_all_persons_paginated_endpoint_success(self, client_player, test_db):
    ...
    async def test_get_all_persons_paginated_endpoint_invalid_page(self, client_player):
        """Test paginated endpoint rejects invalid page number."""
        response = await client_player.get("/api/persons/paginated?page=0")

        assert response.status_code == 422

    async def test_get_all_persons_paginated_endpoint_invalid_items_per_page(self, client_player):
        """Test paginated endpoint rejects invalid items_per_page."""
        response = await client_player.get("/api/persons/paginated?items_per_page=101")

        assert response.status_code == 422

    async def test_get_all_persons_paginated_endpoint_default_sorting(self, client_player, test_db):
    ...
    async def test_get_all_persons_paginated_endpoint_descending_order(self, client_player, test_db):
        ...
        response = await client_player.get("/api/persons/paginated?order_by=second_name&ascending=false")
    ...
    async def test_get_all_persons_paginated_endpoint_multiple_pages(self, client_player, test_db):
        ...
        page1 = await client_player.get("/api/persons/paginated?page=1&items_per_page=2")
        page2 = await client_player.get("/api/persons/paginated?page=2&items_per_page=2")
        page3 = await client_player.get("/api/persons/paginated?page=3&items_per_page=2")
    ...
    async def test_get_persons_count_endpoint(self, client_player, test_db):
        ...
        response = await client_player.get("/api/persons/count")

        assert response.status_code == 200
        assert response.json() == {"total_items": 3}

    async def test_get_persons_count_endpoint_empty(self, client_player, test_db):
    ...
    async def test_existing_get_all_persons_endpoint_still_works(self, client_player, test_db):
    ...
    async def test_get_all_persons_paginated_search_success(self, client_player, test_db):
        ...
        response = await client_player.get("/api/persons/paginated?search=Johnson")
    ...
    async def test_get_all_persons_paginated_with_metadata(self, client_player, test_db):
        ...
        response = await client_player.get("/api/persons/paginated?search=Test&page=1&items_per_page=2")
    ...
    async def test_get_all_persons_paginated_search_no_results(self, client_player, test_db):
        ...
        response = await client_player.get("/api/persons/paginated?search=XYZNonExistent")
    ...
    async def test_get_all_persons_paginated_search_with_pagination(self, client_player, test_db):
        ...
        page1 = await client_player.get("/api/persons/paginated?search=Searchable&page=1&items_per_page=2")
        page2 = await client_player.get("/api/persons/paginated?search=Searchable&page=2&items_per_page=2")
    ...
    async def test_get_all_persons_paginated_empty_search_all_results(self, client_player, test_db):
        ...
        response = await client_player.get("/api/persons/paginated?search=")

        assert response.status_code == 200
        data = response.json()
        assert len(data["data"]) >= 2

    async def test_get_persons_not_in_sport_endpoint(self, client_player, test_db):
        ...
        response = await client_player.get(f"/api/persons/not-in-sport/{sport.id}")
    ...
    async def test_get_persons_not_in_sport_with_search(self, client_player, test_db):
        ...
        response = await client_player.get(f"/api/persons/not-in-sport/{sport.id}?search=Alice")
    ...
    async def test_get_all_persons_not_in_sport_endpoint(self, client_player, test_db):
        ...
        response = await client_player.get(f"/api/persons/not-in-sport/{sport.id}/all")
