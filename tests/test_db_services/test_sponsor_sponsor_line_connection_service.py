...
class TestSponsorSponsorLineServiceDB:
    ...
    async def test_get_sponsor_sponsor_line_relation(self, test_db):
        ...
        result = await connection_service.get_sponsor_sponsor_line_relation(sponsor.id, sponsor_line.id)
    ...
    async def test_get_related_sponsors(self, test_db):
        ...
        connection_service = SponsorSponsorLineServiceDB(test_db)
        await connection_service.create(SponsorSponsorLineSchemaCreate(sponsor_id=sponsor1.id, sponsor_line_id=sponsor_line.id, position=1))
        await connection_service.create(SponsorSponsorLineSchemaCreate(sponsor_id=sponsor2.id, sponsor_line_id=sponsor_line.id, position=2))
    ...
    async def test_delete_relation_not_found(self, test_db):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await connection_service.delete_relation_by_sponsor_and_sponsor_line_id(99999, 99999)
