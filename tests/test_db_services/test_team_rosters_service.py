...
@pytest.mark.asyncio
class TestTeamRostersService:
    async def test_get_team_rosters_success(self, test_db):
        ...
        result = await ptt_service.get_team_rosters_for_match(
            match.id, include_available=True, include_match_players=True
        )

        assert result is not None
        assert result["match_id"] == match.id
        assert len(result["home_roster"]) == 1
        assert len(result["away_roster"]) == 0
        assert result["home_roster"][0]["player_id"] == player1.id
        assert result["home_roster"][0]["is_home_team"] is True
        assert len(result["available_home"]) == 0
        assert len(result["available_away"]) == 1
        assert result["available_away"][0]["player_id"] == player2.id

    async def test_get_team_rosters_match_not_found(self, test_db):
        ptt_service = PlayerTeamTournamentServiceDB(test_db)
        result = await ptt_service.get_team_rosters_for_match(99999)
        assert result is None

    async def test_get_team_rosters_no_match_players(self, test_db):
        sport_service = SportServiceDB(test_db)
        sport = await sport_service.create(SportFactorySample.build())
        ...
        result = await ptt_service.get_team_rosters_for_match(
            match.id, include_available=True, include_match_players=False
        )

        assert result is not None
        assert result["match_id"] == match.id
        assert len(result["home_roster"]) == 0
        assert len(result["away_roster"]) == 0
        assert len(result["available_home"]) == 1
        assert len(result["available_away"]) == 0

    async def test_get_team_rosters_no_available(self, test_db):
        sport_service = SportServiceDB(test_db)
        sport = await sport_service.create(SportFactorySample.build())
        ...
        result = await ptt_service.get_team_rosters_for_match(
            match.id, include_available=False, include_match_players=True
        )
