...
def mock_user_service():
    """Mock user service."""
    with patch("src.users.db_services.UserServiceDB.mark_stale_users_offline") as mock:
        mock.return_value = 0
        yield mock


@pytest.fixture
def mock_clock_orchestrator():
    """Mock clock orchestrator."""
    with patch("src.main.clock_orchestrator") as mock:
...
def mock_ws_manager():
    ...
    with patch("src.main.ws_manager") as mock:
        ...
        mock._connection_retry_task = None
        yield mock
...
async def test_lifespan_startup(
    ...
    mock_ws_task = asyncio.create_task(noop_coroutine())
    mock_ws_task.cancel = Mock()
    ...
    with (
        ...
        patch("src.main.mark_stale_users_offline_task", AsyncMock()),
        patch("src.main.cleanup_stale_websocket_connections_task", AsyncMock()),
    ):
        mock_ws_manager._connection_retry_task = mock_ws_task
...
@pytest.mark.asyncio
async def test_lifespan_shutdown(mock_settings, mock_db, mock_ws_manager, mock_clock_orchestrator):
